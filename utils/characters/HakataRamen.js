...
export class HakataRamen {
  ...
  characterChangeMessage(fromCharacter) {
    const messages = [
      `🍜 よっしゃー！${fromCharacter}より博多弁がいいっちゃろ？山笠があるけん、博多たい💪`,
      `🏮 ${fromCharacter}から博多弁に変わったけんね！オッショイ！オッショイ〜✨`,
      `🍜 ${fromCharacter}お疲れ様！今度は博多弁の出番やけんね🎊`,
  ...
  info(message, data = null) {
    const messages = [
      ...
      `🏃‍♂️ ${message}たい！勢いつけてガンガン行くばい〜🎊`,
      `🌅 ${message}ばい。博多湾の夕日みたいに清々しかろうが！💪`
    ];
  ...
  success(message, data = null) {
    const messages = [
      ...
      `💪✨ ${message}ばい！博多っ子も納得の出来栄えやん！🎊 天神の博多だるまでラーメン食いに行かんね？🍜`,
      `🍜🎉 ${message}！一蘭レベルの完璧さたい〜👑 ちなみに博多駅の一蘭は24時間営業で便利ばい💪`,
      `🏃‍♂️💨 ${message}！福砂屋レベルでピシャッと決まったばい🎊✨ 天神地下街の博多うまかもん市場もおすすめやけんね`,
      `🎯 ${message}ばってん、見事なもんばい！🎊 川端ぜんざいの白玉ぜんざいも絶品たい〜😋`,
      `⚡ ${message}やけん、さすがばい！💪 キャナルシティの博多ラーメン劇場で食べ比べしたかね〜`,
      `🌟 ${message}しとぉ〜完璧ばい！✨ 長浜ラーメンの元祖長浜屋も朝から営業しとぉけん行ってきんしゃい🍜`,
  ...
  error(message, data = null) {
    const messages = [
      `🍜 あちゃ〜、${message}でエラーがでとるったい。博多っ子に任せとかんね〜💪`,
      `😅 ${message}でコケたったい...替え玉頼んでからもう一回挑戦するけん！🍜`,
      `💥 うわ〜！${message}で大コケたい！でも、何回転んでも起き上がるけんね〜🏃‍♂️`,
      `🌧️ ${message}...雨の日のどんたくみたいに、今日はついとらんばい💦でも、明日は晴れるけん✨`,
  ...
  debug(message, data = null) {
    const messages = [
      `🔍🍜 ${message} *博多っ子の探偵スピリット* 細かいところまで見るけんね〜💪`,
      `🕵️‍♂️ ${message} *山笠の職人魂で精査* 隅々まで調べるけん✨`,
      `🔧🏮 ${message} *博多の職人気質* 完璧になるまで調べ尽くすったい💫`,
      `📊🍜 ${message} *豚骨スープの味見みたいに* じっくり確認するけんね〜😊`,
  ...
  getRamenLevel(duration) {
    if (duration < 100) {
      ...
      return {
        ...
        comment: '🏮 博多やったら「バリカタ」が一番注文されるけん、これが正解ばい✨',
        showArt: false
      };
      ...
      return {
        ...
        comment: '😌 博多のおじいちゃん、おばあちゃんが好む優しい硬さやん',
        showArt: false
      };
